...
=======
# Model Diagnostics (Exact Invariance Test)
To ensure our model is not prone to a computational error and our computed results are not due luck, we perform the exact invariance test in this section of the project. We modify the code used in the [STAT 447 Lecture Notes](https://ubc-stat-ml.github.io/web447/w12_mcmc2/topic08_debug.html). 

We use the `arima.sim()` function to generate our synthetic data in the forward simulated using the model found in the EDA section of this project.

```{r, echo = FALSE}
forward = function() {
  x = arima.sim(n = 1, list(order = c(0, 1, 1)))
  return(list(x = x))
}
...
#test_result = exact_invariance(mcmc)
#test_result
```
...
Our project is largely limited to the short amount of time that we are able to spend developing these models and lack of available computation resources at our disposal. With enough hyperparameter tuning, we expect that our models would be capable of performing much better and providing more accurate results.
>>>>>>> b484b42b9e1124bff26883acd266eba2a0c419fb
