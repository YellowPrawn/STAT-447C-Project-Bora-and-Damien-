...
# Obtaining testing data (Since data for 3 years is a subset of 5 years, the data that exists within the 2024/3/1 and 2024/4/8 should be the same across both datasets)
...
x.ts.5.dst <- x.ts.5.train - x.ts.5.stl$time.series[,"seasonal"] - x.ts.5.stl$time.series[,"trend"]
...
```{r}
sarima.model.3 <- arima(x.ts.3.train, order=c(0,1,0), seasonal=list(order=c(0,1,1), period = 182),include.mean = FALSE) # use period of 91 to represent a biannual fluctuation.
sarima.model.3

sarima.model.5 <- arima(x.ts.5.train, order=c(0,1,0), seasonal=list(order=c(0,1,1), period = 182),include.mean = FALSE) # use period of 182 to represent a biannual fluctuations
sarima.model.5
```

```{r, echo = FALSE}
sarima.3.preds <- predict(sarima.model.3, n.ahead = 39, prediction.interval = TRUE)
sarima.5.preds <- predict(sarima.model.5, n.ahead = 39, prediction.interval = TRUE)
```

```{r, echo = FALSE, out.width="100%", out.height="30%", fig.width=8, fig.height=6}

time <- seq.POSIXt(as.POSIXlt("2024-03-01"), as.POSIXlt("2024-04-09"), by="day")
time <- as.Date(format.POSIXct(time,'%Y-%m-%d'))

plot(time, x.ts.test, ylab = "S&P 500 Index", col = "#000000", ylim = c(4000, 6000), type = 'l', main = "SARIMA model predictions compared to test data")
lines(time, sarima.3.preds$pred, col = "#FF0000", type = 'l')
lines(time, sarima.3.preds$pred - qnorm(0.975) * sarima.3.preds$se, col = "#FF0000", lty = "dashed", type = 'l')
lines(time, sarima.3.preds$pred + qnorm(0.975) * sarima.3.preds$se, col = "#FF0000", lty = "dashed", type = 'l')
lines(time, sarima.5.preds$pred, col = "#0000FF", type = 'l')
lines(time, sarima.5.preds$pred - qnorm(0.975) * sarima.5.preds$se, col = "#0000FF", lty = "dashed", type = 'l')
lines(time, sarima.5.preds$pred + qnorm(0.975) * sarima.5.preds$se, col = "#0000FF", lty = "dashed", type = 'l')
...
```{r, include = FALSE}
mean((sarima.3.preds$pred - x.ts.test)^2)
mean((sarima.5.preds$pred - x.ts.test)^2)
```

To compare the SARIMA model's performance with the bayesian approach our project demonstrates, we compute the mean squared prediction error (MSPE) of both SARIMA models to be $32688.32$ for the model trained on the 3 year dataset, and $36738.45$ for the model trained on the 5 year dataset. We will compute the MSPE for the MCMC model later in this report.
...
$AR(1)$ process is suitable for the data too. Therefore, we incorporate this knowledge into our 
Bayesian model. Let $i$ be the time index and $y$ be the deseasonalized time 
series. 
...
We use the data from April $2021$ to Feburary $2024$ for training. Further, 
the data from March $2024$ to April $2024$ will be used for testing.
...
```{r, include = FALSE, out.width="33%", out.height="150%"}
samples = extract(fit)
fit
yhats = colMeans(samples$yhat)
yhats.se = apply(samples$yhat, 2, sd)
```
...
preds.se.ts = ts(yhats.se, 
              start=c(2024, as.numeric(format(as.Date("2024-03-01"), "%j"))), 
              end=c(2024, as.numeric(format(as.Date("2024-04-08"), "%j"))), 
              frequency = 365)

plot(time, x.ts.test, ylab = "S&P 500 Index", col = "#000000", ylim = c(4000, 6000), type = 'l', main = "MCMC model predictions VS test data VS SARIMA")

lines(time, preds.ts, col = "purple", type = 'l')
lines(time, preds.ts - qnorm(0.975) * preds.se.ts, col = "purple", lty = "dashed", type = 'l')
lines(time, preds.ts + qnorm(0.975) * preds.se.ts, col = "purple", lty = "dashed", type = 'l')
...
legend("topleft", legend = c("Test Data", "MCMC Model", "SARIMA (3 years)", "SARIMA (5 years)", "PI Bounds"), col = c("#000000", "purple", "#FF0000", "#0000FF", "#000000"), lty = c("solid", "solid", "solid", "solid", "dashed"))
```

```{r, include = FALSE}
mean((preds.ts - x.ts.test)^2)
```

This plot suggests that the simple MCMC model with 3 years of data matches the performance of the SARIMA models. It achieves $MSPE = 8591.045$ which is significantly better than the MSPE for the SARIMA models. However, it it can be observed that the prediction interval seems to be larger. The error of the model is not improved compared to the frequentist approach.
...
COVID times. We assume that there are four change points, and therefore five 
$\alpha$ values corresponding the divided regions. Further, we assume that 
...
, where $K_1$ only modifies $\sigma$, $K_2$ only modifies the $\alpha$'s, and
...
$K_2$ can reach all of $R_{+}^5$ from any starting point. Lastly, $K_3$ is irreducible
if the proposal for $K_3$ can reach all of 
$$\{1,2,..., 447\} \times \{448, 315,..., 894\} \times \{895, 628, ..., 1341\} \times \{1342, 941, ..., 1788\} $$
from any starting point. For the sigma, we use a normal distribution centered at
the current $\sigma$ as the proposal distribution:
$$\sigma' \sim N(\sigma, 5)$$
For $\alpha$'s we will use a multivariate normal distribution centered at the 
current $\alpha$'s as the proposal distribution :

$$\alpha' \sim N(\alpha, 0.001)$$
Lastly, for each change point, we use a discrete uniform distribution as the proposal

$c_1' \sim Unif\{1, 447\}$, ,$c_2' \sim Unif\{447, 894\}$, $c_3' \sim Unif\{895, 1341\}$,
$c_4' \sim Unif\{1342, 1788\}$.
By our choice of proposals, the MCMC can reach any point in 
$\{1,2,..., 447\} \times \{448, 315,..., 894\} \times \{895, 628, ..., 1341\} \times \{1342, 941, ..., 1788\}$

Let $\rho$ be the parameter we used to determine which kernel to use, we define it as :

$$\rho \sim Unif\{1,3\}$$
if $\rho = 1$, we use $K_1$, if $\rho = 2$ we use $K_2$ and if $\rho = 3$, we 
use $K_3$. Let $\alpha = (\alpha_1, \alpha_2, \alpha_3, \alpha_4, \alpha_5)$ and
$c = (c_1, c_2, c_3, c_4)$. In each $K_1$ we define the ratio 
as $\frac{\gamma(\sigma', \alpha,c )}{\gamma(\sigma, \alpha,c)}$, in $K_2$ as 
: $\frac{\gamma(\sigma, \alpha',c )}{\gamma(\sigma, \alpha,c)}$, in $K_3$ as:
 $\frac{\gamma(\sigma, \alpha,c' )}{\gamma(\sigma, \alpha,c)}$ 
 Then let $R^{(m)}$ be the ratio, we sample  $A^{(m)} ~ \sim Bern(\min\{1, R^{(m)}\})$ 
 and then accept the proposal if $A = 1$, and set $X^{(m)} = \tilde{X}^{(m)}$. If 
$A^{(m)}= 0$, we stay at the current point $X^{(m)} = X^{(m-1)}$.
...
kernel3 = function(gamma, sigma, alphas, current_point, y) {
  ...
  proposal2 = sample(seq(floor(length(y)/4) + 1, 2*floor(length(y)/4)), size = 1)
  proposal3 = sample(seq(2*floor(length(y)/4) + 1, 3*floor(length(y)/4)), size = 1)
  proposal4 = sample(seq(3*floor(length(y)/4) + 1, length(y)), size = 1)
...
```{r, include = FALSE}
set.seed(12)
output = suppressWarnings(mcmc(log_joint, 45, c(1,1,1,1,1), c(365, 730, 1095, 1460), x.ts.5.train, 20000))
```


```{r, include = FALSE}
# Inspired by STAT 447 lecture notes https://github.com/UBC-Stat-ML/web447/blob/main/blocks/plot_traces_and_hist.R
plot_traces_and_hist = function(item, item_name) {
  ...
  output <- output[[item]][output[[item]] != -1]
  
  plot(output, 
     col = rgb(red = 0, green = 0, blue = 0, alpha = 0.2), 
     type = "o",
     main = paste(item_name, "trace plot"),
     xlab = "MCMC Iteration",
     ylab = "p")
  
  xhist <- hist(output, plot = FALSE)
...
hist(output$change_point_trace1[output$change_point_trace1 != -1], xlab = "Time Index", main = "Trace Plot of Change Point 1")
hist(output$change_point_trace2[output$change_point_trace2 != -1], xlab = "Time Index", main = "Trace Plot of Change Point 2")
hist(output$change_point_trace3[output$change_point_trace3 != -1], xlab = "Time Index", main = "Trace Plot of Change Point 3")
hist(output$change_point_trace4[output$change_point_trace4 != -1], xlab = "Time Index", main = "Trace Plot of Change Point 4")
```

The spike in the plot for the first change point indicates that the change 
change point is around March $2023$, when COVID restriction started, which is 
aligned with our expectations. The second change point seems to be between 
$800$th and $900$th time indices, which corresponds to around August $2021$, 
. Further, the third change point seem to be around $950$th or $1350$th time index, 
which is also aligned with the plot. There seems to be three potential candidates
for the fourth change point, the most notable one being around $1600th$ and $1700$th
time indices, which corresponds to around October $2023$. Therefore, the fourth 
change point is also aligned with the plot of the five year time series.
...
It seems that the standard deviation $38.56$ is less than the Bayesian 
standard deviation estimate for the three year data $45.46$. Further, by using
a change point model using kernel mixtures, we had more flexibility in terms of
having Bayesian estimates of the $\alpha$ values for each region determined by 
the change points. The highest $\alpha$ value was chosen for the fifth region,
which is the one that is on the most right hand side. The first and the fourth 
regions has the smallest $\alpha$ values.


We make predictions with the change point model as follows. We use the $\alpha$
value corresponding to the rightmost region.

```{r, echo = FALSE}
set.seed(1)
alpha5.hat = mean(output$alpha5_trace[output$alpha5_trace != -1])
sigma.hat = mean(output$sigma_trace[output$sigma_trace != -1])
yhats.5 = rep(-1, length(x.ts.test))
for (i in 1:length(x.ts.test)){
  if(i == 1){
     yhats.5[i] = rnorm(1, alpha5.hat*x.ts.5.train[length(x.ts.5.train)], sigma.hat)
  } else{
    yhats.5[i] = rnorm(1, alpha5.hat*yhats.5[i-1], sigma.hat)
  }
}
...
```{r, echo = FALSE, out.width="100%", out.height="30%", fig.width=8, fig.height=6}
cı.yhats.5 = quantile(yhats.5, c(0.025, 0.975))
plot(time, x.ts.test, ylab = "S&P 500 Index", col = "#000000", ylim = c(4000, 6000), type = 'l', main = "Change Point Model VS Previous Models")

lines(time, yhats.5, col = "green", type = 'l')
lines(time, yhats.5 - qnorm(0.975) * sigma.hat, col = "green", lty = "dashed", type = 'l')
lines(time, yhats.5 + qnorm(0.975) * sigma.hat, col = "green", lty = "dashed", type = 'l')
...
legend("topleft", legend = c("Test Data", "Change Point Model", "MCMC Model", "SARIMA (3 years)", "SARIMA (5 years)", "PI Bounds"), col = c("#000000", "green", "purple", "#FF0000", "#0000FF", "#000000"), lty = c("solid", "solid", "solid", "solid", "solid", "dashed"))
```
```{r, include = FALSE}
print(mean((yhats.5 - x.ts.test)^2))
```
...
```{r, include = FALSE}
set.seed(12)
output.quarter = suppressWarnings(mcmc(log_joint, 11.25, c(1,1,1,1,1), c(365, 730, 1095, 1460), x.ts.5.train, 20000))

alpha5.hat.quarter = mean(output.quarter$alpha5_trace[output.quarter$alpha5_trace != -1])
sigma.hat.quarter = mean(output.quarter$sigma_trace[output.quarter$sigma_trace != -1])
yhats.5.quarter = rep(-1, length(x.ts.test))
for (i in 1:length(x.ts.test)){
  if(i == 1){
     yhats.5.quarter[i] = rnorm(1, alpha5.hat.quarter*x.ts.5.train[length(x.ts.5.train)], sigma.hat.quarter)
  } else{
    yhats.5.quarter[i] = rnorm(1, alpha5.hat.quarter*yhats.5.quarter[i-1], sigma.hat.quarter)
  }
}
```
...
```{r, include = FALSE}
set.seed(12)
output.quadruple = suppressWarnings(mcmc(log_joint, 180, c(1,1,1,1,1), c(365, 730, 1095, 1460), x.ts.5.train, 20000))

alpha5.hat.quadruple = mean(output.quadruple$alpha5_trace[output.quadruple$alpha5_trace != -1])
sigma.hat.quadruple = mean(output.quadruple$sigma_trace[output.quadruple$sigma_trace != -1])
yhats.5.quadruple = rep(-1, length(x.ts.test))
for (i in 1:length(x.ts.test)){
  if(i == 1){
     yhats.5.quadruple[i] = rnorm(1, alpha5.hat.quadruple*x.ts.5.train[length(x.ts.5.train)], sigma.hat.quadruple)
  } else{
    yhats.5.quadruple[i] = rnorm(1, alpha5.hat.quadruple*yhats.5.quadruple[i-1], sigma.hat.quadruple)
  }
}
```

```{r, echo = FALSE, out.width="100%", out.height="30%", fig.width=8, fig.height=6}
plot(time, x.ts.test, ylab = "S&P 500 Index", col = "#000000", ylim = c(4000, 6000), type = 'l', main = "Change Point Model with different prior sigmas")

lines(time, yhats.5, col = "green", type = 'l')
lines(time, yhats.5 - qnorm(0.975) * sigma.hat, col = "green", lty = "dashed", type = 'l')
lines(time, yhats.5 + qnorm(0.975) * sigma.hat, col = "green", lty = "dashed", type = 'l')

lines(time, yhats.5.quarter, col = "orange", type = 'l')
lines(time, yhats.5.quarter - qnorm(0.975) * sigma.hat.quarter, col = "orange", lty = "dashed", type = 'l')
lines(time, yhats.5.quarter + qnorm(0.975) * sigma.hat.quarter, col = "orange", lty = "dashed", type = 'l')

lines(time, yhats.5.quadruple, col = "brown", type = 'l')
lines(time, yhats.5.quadruple - qnorm(0.975) * sigma.hat.quadruple, col = "brown", lty = "dashed", type = 'l')
lines(time, yhats.5.quadruple + qnorm(0.975) * sigma.hat.quadruple, col = "brown", lty = "dashed", type = 'l')

legend("topleft", legend = c("Test Data", "sigma = 45 (Original)", "sigma = 11.25", "sigma = 180", "PI Bounds"), col = c("#000000", "green", "orange", "brown", "#000000"), lty = c("solid", "solid", "solid", "solid", "dashed"))
```


```{r, include = FALSE}
mean((yhats.5.quarter - x.ts.test)^2)
mean((yhats.5.quadruple - x.ts.test)^2)
```

The MSPE of the models with different $sigma$ priors (11.25 and 180) are relatively close to that of the original prior choice of $\sigma = 45$. $\sigma = 11.25$ obtained $MSPE = 7293.212$ and $\sigma = 180$ obtained $MSPE = 7334.161$. Although both are close to that of $\sigma = 45$ at $MSPE = 6859.014$, we can observe on the graph that the predictions from $\sigma = 11.25$ are quite rigid, and do not account for the variation in the time series quite as well; this is naturally expected as the chosen standard deviation is lower. Conversely, $\sigma = 180$ does not have a prediction interval that is significantly larger than that of $\sigma = 45$, but, when the point estimates in $\sigma = 180$ deviate away from the true values of the data, they seems to deviate much more than the predictions seen in $\sigma = 45$. This suggests that the prediction error for values further in-the-future would be significantly higher. Overall, the model's point estimate predictions are not very sensitive to prior changes as demonstrated here: only the prediction intervals are significantly sensitive as expected by definition.


# Model Diagnostics 

We would like to ensure that the MCMC model is fast mixing. Therefore, 
we present trace plots obtained by using different chains. To create the 
second chain, we utilize a different random seed.

```{r, echo = FALSE}
set.seed(1)
output2 = suppressWarnings(mcmc(log_joint, 45, c(1,1,1,1,1), c(365, 730, 1095, 1460), x.ts.5.train, 20000))
```
...
plot_traces_and_hist = function(item, item_name) {
  ...
  output_1 <- output[[item]][output[[item]] != -1]
  output_2 <- output2[[item]][output2[[item]] != -1]
  
  plot(output_1, 
     col = rgb(red = 0, green = 0, blue = 0, alpha = 0.2), 
     type = "o",
     main = paste("Trace plot for change point kernel model: ", item_name),
     xlab = "MCMC Iteration",
     ylab = "p")
  points(seq(1,length(output_2)), 
       output_2, 
       col = rgb(red = 1, green = 0, blue = 0, alpha = 0.2), 
       type = "o")
  
  xhist <- hist(output_1, plot = FALSE)
  xhist_2 <- hist(output_2, plot = FALSE)
  barplot(xhist$counts, 
          axes = TRUE, 
          space = 0, 
          horiz=TRUE,
          col = rgb(red = 0, green = 0, blue = 0, alpha = 0.2))
  barplot(xhist_2$counts, 
          axes = TRUE, 
          space = 0, 
          horiz=TRUE,
          col = rgb(red = 1, green = 0, blue = 0, alpha = 0.2),
