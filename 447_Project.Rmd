...
standard devation estimate for the three year data $45.46$. Further, by using
...
```{r, echo = FALSE}
plot(time, x.ts.test, ylim =  c(5050, 5300) , main = "Actual Values vs Predictions by the Change Point Model")
lines(time, yhats.5, col = "purple")
```

Evidently, the prediction performance has improved compared to the frequentist 
and the simple bayesian model. Further, the model has gained flexibility.


=======
# Model Diagnostics 

We would like to make sure that the MCMC model is mixing fastly. Therefore, 
we will create trace plots obtained by using different chains. To create the 
second chain, we are using a different seed.
...
```{r, echo = FALSE}
plot(output$sigma_trace[output$sigma_trace != -1])
points(seq(1,length(output2$sigma_trace[output2$sigma_trace != -1])), 
       output2$sigma_trace[output2$sigma_trace != -1], col = "red")
plot(output$alpha1_trace[output$alpha1_trace != -1])
points(seq(1,length(output2$alpha1_trace[output2$alpha1_trace != -1])), 
       output2$alpha1_trace[output2$alpha1_trace != -1], col = "red")
plot(output$alpha2_trace[output$alpha2_trace != -1])
points(seq(1,length(output2$alpha2_trace[output2$alpha2_trace != -1])), 
       output2$alpha2_trace[output2$alpha2_trace != -1], col = "red")
plot(output$alpha3_trace[output$alpha3_trace != -1])
points(seq(1,length(output2$alpha3_trace[output2$alpha3_trace != -1])), 
       output2$alpha3_trace[output2$alpha3_trace != -1], col = "red")
plot(output$alpha4_trace[output$alpha4_trace != -1])
points(seq(1,length(output2$alpha4_trace[output2$alpha4_trace != -1])), 
       output2$alpha4_trace[output2$alpha4_trace != -1], col = "red")
plot(output$alpha5_trace[output$alpha5_trace != -1])
points(seq(1,length(output2$alpha5_trace[output2$alpha5_trace != -1])), 
       output2$alpha5_trace[output2$alpha5_trace != -1], col = "red")

```

The MCMC model seems to be mixing well as the trace plots obtained from different 
chains are hard to distinguish. 


# Conclusion

In conclusion, both the Frequentist and the Bayesian approaches we have taken to modelling and forecasting the S&P 500 Index achieve a similar level of performance. However, when Kernel Mixtures are implemented into the MCMC algorithm, we are more likely to be able to handle sudden changes and fluctuations in the time series; in other words, the model which incorporates Kernel Mixtures is significantly more robust. Since it is easier to describe how things change rather than how things are, the change points model we have developed should help with future crisis in terms how much the index will deviate from its performance in normality.


## Limitations

Our project is largely limited to the short amount of time that we are able to spend developing these models and lack of available computation resources at our disposal. With enough hyperparameter tuning, we expect that our models would be capable of performing much better and providing more accurate results. Further, incorporating further tools for the model to use would be useful, such as recent news or events that can significantly impact values. This could be FED interest rates, new pandemic, new war. By incorporating these tools, the model could be more dynamic.
