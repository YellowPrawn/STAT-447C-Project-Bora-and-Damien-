...
Now, we will use the five year data that includes point, and will try to find 
the change point. From the time series plot of the five year data, it seems that 
...
log_joint = function(alphas, change_point, y) {
  
  # Return log(0.0) if parameters are outside of the support
  if (alphas[[1]] < 0 | alphas[[2]] < 0 | alphas[3] < 0) 
    return(-Inf)
  ...
  for (i in 1:length(y)) {
    ...
    if (i == 1){
      log_likelihood = log_likelihood + dnorm(y[[i]], alpha*y[[i]], 45.46, log = TRUE)
    } else{
      log_likelihood = log_likelihood + dnorm(y[[i]], alpha*y[[i-1]], 45.46, log = TRUE)
...
```{r}
kernel2 = function(gamma, current_point, proposal_sd, change.p, y) {
dim = length(current_point)
proposal = rnorm(dim, mean = current_point, sd = proposal_sd)
ratio = gamma(proposal, change.p,y) / gamma(current_point, change.p, y)
if (runif(1) < ratio & ratio != Inf) {
return(proposal)
} else {
return(current_point)
...
```{r}
kernel3 = function(gamma, alphas, current_point, y) {
dim = length(current_point)
proposal1 = sample(1:(length(y)/2), size = 1)
proposal2 = sample((length(y)/2) : length(y), size = 1)
proposal = c(proposal1, proposal2)
ratio = gamma(alphas, proposal, y) / gamma(alphas, current_point, y)
if (runif(1) < ratio & ratio != Inf) {
return(proposal)
} else {
return(current_point)
...
mcmc = function(gamma, alphas, change_point, y, n_iterations){
  change_point_trace1 = rep(-1, n_iterations)
  change_point_trace2 = rep(-1, n_iterations)
  for (i in 1:n_iterations) {
    rho = rbinom(1,1,1/2) # choose kernel
    if(rho == 0){
      alphas =  kernel2(log_joint, alphas, 0.01, change_point, y)
      change_point_trace1[i] = change_point[1]
      change_point_trace2[i] = change_point[2]
      
} 
    else{
      change_point = kernel3(log_joint, alphas, change_point, y)
       change_point_trace1[i] = change_point[1]
      change_point_trace2[i] = change_point[2]
...
```{r}
output = suppressWarnings(mcmc(log_joint, c(1,1, 1), c(200, 700), x.ts.5.train, 10000))
```
